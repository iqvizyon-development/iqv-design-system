...
Iqvizyon UI strives to find a balance that enables consumers who support modern browsers to reap the
...
Iqvizyon UI fully supports browsers that are at or above the indicated versions below. Consumers who support these browsers
are poised to experience the full performance and bundlesize benefits that these modern browsers afford the Iqvizyon UI React library.
...
Iqvizyon UI will ensure that no browser or CSS features that are incompatible with the support matrix laid out above are introduced. The same goes for
ECMAScript features, Iqvizyon UI will be targeting ES2020 and thus will be shipping an ES2020 compliant code which will be fully compatible with the
...
Iqvizyon UI introduces a new concept of a partial browser support matrix as a means to find that balance between supporting modern browsers and older browsers.
...
List of features that are currently used in Iqvizyon UI that aren't supported out of the box by the partial support matrix above:
...
- As previously mentioned, Iqvizyon UI will be targeting ES2020. Some ES2020 features are already present
  ...
  will need to transpile down the Iqvizyon UI library to make it work since the use of these ES2020 features results in the application crashing on the listed browsers below.
...
By default Iqvizyon UI will not be providing polyfills for features we expect our full browser support matrix to already support.
The only instance that a polyfill may be provided is when Iqvizyon UI's use of a feature causes an application to crash on
...
- handling polyfills on the application level ensures that those on modern browsers don't suffer from unnecessary overhead of
  polyfills being shipped with the Iqvizyon UI library by default
...
1. Iqvizyon UI developers and contributors are internally expected to develop against the partial browser support matrix.
   ...
   should be present in the library. Should any application breaking errors be discovered in the future, Iqvizyon UI will be responsible for fixing it internally.
2. Since Iqvizyon UI will be transpiling down and shipping ES2020 code, Iqvizyon UI developers will internally develop against the latest ECMA standards.
3. Iqvizyon UI developers and contributors reserve the right to use features that fall in between the partial and full browser support matrices (like flex gap) as
   long as they don't result in applications crashing.

## Browser Support Going Forward

With an eye towards being a modern library, Iqvizyon UI will follow a yearly audit process to evaluate and update the current browser support matrix to keep pace
...
consumers with performance and bundlesize improvements. These yearly audits will give Iqvizyon UI a reasonable timeframe to evaluate, decide and give notice if browser support
changes are made.
