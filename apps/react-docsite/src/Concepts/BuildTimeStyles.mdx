...
Iqvizyon UI uses the open source [Griffel.js](https://griffel.js.org/) CSS-in-JS styling engine.
Make sure to take a look at the following docs to get started with styling in Iqvizyon UI:
...
applications might want to optimize for performance. In Iqvizyon UI, the expensive runtime work only
...
it's necessary to install `@babel/preset-typescript` and configure it in the loader's `babelOptions`. Similarly,
...
module source. By default, the preset is hard coded to handle imports from `@iqvizyonui/react-components`.
