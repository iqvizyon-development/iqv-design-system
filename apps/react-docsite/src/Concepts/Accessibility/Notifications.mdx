...
## Using Iqvizyon's `useAnnounce` hook

There are multiple advantages to using the built-in Iqvizyon `AriaLiveAnnouncer` + `useAnnounce` hook to handle live regions instead of building your own in the DOM:
...
This applies to custom DOM-based live regions, not to the Iqvizyon announce utility.
...
Instead, use the Iqvizyon `useTypingAnnounce` hook, which will both batch and debounce any `typingAnnounce` calls and fire a single announcement 0.5s after the user ceases typing.
