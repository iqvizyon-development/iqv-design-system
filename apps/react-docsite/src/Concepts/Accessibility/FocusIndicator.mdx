...
import { IqvizyonCanvas, IqvizyonStory } from '@iqvizyonui/react-storybook-addon';

<Meta title="Concepts/Developer/Accessibility/Focus indicator" />

## Focus indicator

Iqvizyon UI components use `tabster` for focus handling functionality, so that they can be integrated with application-level focus management such as delooser and cross-iframe focusing.

Iqvizyon UI `@iqvizyonui/react-tabster` package defines `useKeyboardNavAttribute`, `createCustomFocusIndicatorStyle` and `createFocusOutlineStyle` to integrate tabster keyboard navigation mechanism seemlesly within Iqvizyon UI.

### useKeyboardNavAttribute

Instantiates `keyborg` and adds the `data-keyboard-nav` attribute to a referenced element to keep keyboard navigation awareness synchronized without rerendering the React tree.
...
The default focus indicator used in Iqvizyon UI is an outline. However, in some cases more specific focus indicators are necessary depending on the use case and component design. In order to accommodate these requirements, Iqvizyon UI exports two different utilities to style focus indicators:
...
<IqvizyonCanvas>
  <IqvizyonStory id="components-accordion--default" height={260} />
</IqvizyonCanvas>

```tsx
import { makeStyles } from '@iqvizyonui/react-components';
import { createFocusOutlineStyle } from '@iqvizyonui/react-components';
...
<IqvizyonCanvas>
  <IqvizyonStory id="components-link--default" height={150} />
</IqvizyonCanvas>

```tsx
import { makeStyles, createCustomFocusIndicatorStyle } from '@iqvizyonui/react-components';
