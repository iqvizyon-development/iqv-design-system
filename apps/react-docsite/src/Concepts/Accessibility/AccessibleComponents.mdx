...
Iqvizyon UI components are designed to support various aspects of accessibility by default, so that they can be used with different input methods (mouse, touch, keyboard, screen readers) as well as fulfill different rendering and layout requirements (theming, zoom, contrast).
...
Iqvizyon UI components aim to fully respect both WCAG and ARIA practices, the Iqvizyon UI team is working with the community to improve them.
...
Iqvizyon UI components fulfill following requirements:
...
Iqvizyon UI components use `tabster` for focus handling and integration with application-level focus management such as delooser and cross-iframe focusing.

## Out of Scope

- Internationalization, globalization, keyboard shortcuts and language detection are deliberately not part of Iqvizyon UI and should be handled by the hosting application.
- Focus handling outside the component-level scope must be handled by the application.

## Ensuring accessibility

Iqvizyon UI components will be tested to guarantee standard conformance and usability.
