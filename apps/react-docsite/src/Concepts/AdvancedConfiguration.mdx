...
Having multiple applications using `IqvizyonProvider` on a single web page can cause interop problems when they come from different bundles. Use `IdPrefixProvider` or React 18's `identifierPrefix` to isolate generated IDs between applications.
