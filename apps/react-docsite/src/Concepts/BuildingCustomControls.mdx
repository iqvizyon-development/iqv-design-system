...
A custom control is a component you own entirely — its markup, its styles, its design tokens — that still needs to behave like its Iqvizyon counterpart: correct ARIA roles, keyboard navigation, focus management, and slot structure.

Building one from scratch means reimplementing behavior that Iqvizyon UI has already solved. Base state hooks let you avoid that. They expose the behavior layer of a Iqvizyon component as a standalone API you can call directly, while leaving rendering and styling entirely up to you.

### When to use base state hooks

There are three levels of customization available, depending on how much control you need:

**Styled components** (`Button`, `Input`, etc.) — the default for Iqvizyon UI apps. Use when you want built-in accessibility, design tokens, and standard visual behavior with optional style overrides.

**Composition hooks** (`useButton_unstable`, `useInput_unstable`, etc.) — the right choice for Iqvizyon DS extensions. Use when you need to own rendering but still want Iqvizyon's visual design language as a starting point.
...
- Iqvizyon behavior and accessibility primitives without any styling opinions

Base state hooks are not a replacement for composition hooks in most cases. If you are customizing a Iqvizyon component's appearance or adding design variants, composition hooks are the right starting point.
...
import type { ButtonBaseProps, ButtonBaseState } from '@iqvizyonui/react-button';
...
import { assertSlots, mergeClasses, slot, type Slot } from '@iqvizyonui/react-components';
import { useButtonBase_unstable } from '@iqvizyonui/react-button';
import type { ButtonBaseProps, ButtonBaseState } from '@iqvizyonui/react-button';
...
For deeper rationale and design decisions, see [Component patterns](https://github.com/iqvizyon-development/iqv-design-system/blob/master/docs/architecture/component-patterns.md).
