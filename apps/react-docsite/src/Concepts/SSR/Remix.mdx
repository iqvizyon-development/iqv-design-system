...
```bash
# Install Iqvizyon UI core packages
npm i @iqvizyonui/react-components @fluentui/react-icons
...
export default defineConfig(({ command }) => ({
  plugins: [
    ...
    // Add CJS interop plugin for Iqvizyon UI packages until they are ESM compatible
    cjsInterop({
      dependencies: ['@iqvizyonui/react-components'],
    }),
    // Add Griffel plugin for production optimization
    command === 'build' && griffel(),
  ],
  // Required for Iqvizyon UI icons in SSR
  ssr: {
...
2. Modify `app/root.tsx` to add Iqvizyon UI providers:

```tsx
// 1. Import Iqvizyon UI dependencies
  ...
  return (
    <html lang="en">
      <head>
        ...
        <Meta />
        <Links />
        {/* 2. Add insertion point for Iqvizyon UI styles before the </head>. */}
        <meta name="fluentui-insertion-point" content="fluentui-insertion-point" />
      </head>
      <body>
        {/* 3. Wrap app content with IqvizyonProvider */}
        <IqvizyonProvider theme={webLightTheme}>{children}</IqvizyonProvider>
        <ScrollRestoration />
        <Scripts />
      </body>
    </html>
...
```tsx
import { createDOMRenderer, RendererProvider, SSRProvider } from '@iqvizyonui/react-components';
...
```tsx
// 1. Import required Iqvizyon UI SSR utilities
import { createDOMRenderer, RendererProvider, renderToStyleElements, SSRProvider } from '@iqvizyonui/react-components';
...
export default function handleRequest(
  ...
  return new Promise((resolve, reject) => {
    ...
    const { pipe, abort } = renderToPipeableStream(
      // 5. Wrap RemixServer with Iqvizyon UI providers
      ...
      </RendererProvider>,
      {
        [callbackName]: () => {
          ...
          const body = new PassThrough({
            // 6. Transform stream to inject Iqvizyon UI styles
...
```tsx
import { Button, Card, Title1, Body1 } from '@iqvizyonui/react-components';
import { BookmarkRegular } from '@fluentui/react-icons';

export default function Index() {
  return (
    <Card style={{ maxWidth: '400px', margin: '20px' }}>
      <Title1>Iqvizyon UI + Remix</Title1>
