...
Iqvizyon UI web components and Fluent tokens should be installed as a `dependency` of your app. Both packages are also [available on unpkg](https://unpkg.com/@iqvizyonui/web-components).
...
The easiest way to get up and running with Fluent Web Components is to use our entire component bundle. You will also need Iqvizyon UI's design token CSS on the page in either by generating stylesheet yourself or using our `setTheme` utility.
