...
Iqvizyon UI React v9 offers predefined tokens for creating custom motions. These tokens can be used as CSS variables or plain JavaScript values, depending on the scenario.

```ts
import { createMotionComponent, makeStyles, motionTokens, tokens } from '@iqvizyonui/react-components';
