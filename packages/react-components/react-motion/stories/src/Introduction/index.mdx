...
import { Card, CardHeader, tokens } from '@iqvizyonui/react-components';
...
## What is Iqvizyon Motion?

Iqvizyon Motion is a set of React components that make it easy to animate elements in your UI — showing, hiding, sliding, fading, and more. You describe _what_ you want to animate, and the system handles the _how_.
...
Inspired by [atomic design](https://bradfrost.com/blog/post/atomic-web-design/), Iqvizyon Motion is built in composable layers:
...
Iqvizyon Motion automatically respects the user's **prefers-reduced-motion** operating system setting.
...
The Iqvizyon Motion System uses **design tokens** — named values that keep animation timing consistent across all Microsoft products. Instead of picking arbitrary millisecond values, you choose a token that communicates the _intent_ of the motion.
...
Easing curves control how an animation accelerates and decelerates. Iqvizyon defines three families:
...
The Iqvizyon Motion System distinguishes between two fundamental patterns:
...
| ------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------- |
| `@iqvizyonui/react-motion`                    | Stable | Core factories (`createMotionComponent`, `createPresenceComponent`), `PresenceGroup`, `motionTokens`, `presenceMotionSlot` |
| `@iqvizyonui/react-motion-components-preview` | Beta   | Ready-to-use components: `Fade`, `Scale`, `Collapse`, `Slide`, `Blur`, `Rotate`, `Stagger`, plus Snappy/Relaxed variants   |
