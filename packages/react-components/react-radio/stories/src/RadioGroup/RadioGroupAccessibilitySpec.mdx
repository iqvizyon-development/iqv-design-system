...
| label     | label (native) | A `<label htmlFor={input.id}>` from `@iqvizyonui/react-label`. Clicking it selects the radio.                                                       |

Focus lives on each radio's `<input>` element. The visible focus indicator is drawn on the radio's root via `:focus-within` from `createFocusOutlineStyle` in `@iqvizyonui/react-tabster`, because the actual `<input>` is `opacity: 0`.
