...
| label     | label (native)    | A `<label htmlFor={input.id}>` from `@iqvizyonui/react-label`. Clicking it toggles the input.                                                                                                  |
...
Focus lives on the `<input>` element. The visible focus indicator is drawn on the root via `:focus-within` from `createFocusOutlineStyle` in `@iqvizyonui/react-tabster`, because the actual `<input>` is `opacity: 0` and would not show its native focus ring.
